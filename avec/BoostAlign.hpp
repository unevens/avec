// the code in this file is taken from boost::align and adapted into a single
// header. this was done to avoid having to add boost as a dependency.
...
struct max_size : std::integral_constant<std::size_t, (A > B) ? A : B>
{};
...
class aligned_allocator
{
  static_assert(detail::is_alignment_constant<Alignment>::value,
                "boost::alignment::is_alignment_constant failed");
...
public:
  ...
  pointer allocate(size_type size, const_void_pointer = 0)
  {
    ...
    {
      m =
        detail::max_size<Alignment, std::alignment_of<value_type>::value>::value
    };
    if (size == 0) {
      return 0;
    }
...
inline bool
is_aligned(const volatile void* ptr, std::size_t alignment) noexcept
{
  assert(detail::is_alignment(alignment));
...
inline bool
is_aligned(std::size_t alignment, const volatile void* ptr) noexcept
{
  assert(detail::is_alignment(alignment));
  return (reinterpret_cast<std::size_t>(ptr) & (alignment - 1)) == 0;
}
